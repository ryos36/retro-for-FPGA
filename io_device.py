...
from uart_cmd_server import STATUS_IO_PORT, STATUS_BUSY_FLAG, CMD_IO_PORT, IO_CMD_NOP, IO_CMD_TX_UART, IO_CMD_RX_UART, IO_CMD_TX_FLUSH_UART, IO_CMD_REPLY_FLAG, IO_ID_PORT, UART_IO_PORT

#----------------------------------------------------------------
def wait_freedom( io_ports ):
    while True:
        cmd_reply:bit32 = io_ports[STATUS_IO_PORT] & 0xFFFFFFFF
        if (cmd_reply & STATUS_BUSY_FLAG) == 0:
            break
def send_cmd( io_ports, cmd ):
    io_ports[CMD_IO_PORT] = cmd
    while True:
        cmd_reply:bit32 = io_ports[CMD_IO_PORT] & 0xFFFFFFFF
        if cmd_reply == (IO_CMD_REPLY_FLAG | cmd):
            break
        
def rx_char( io_ports ):
    wait_freedom( io_ports )
    send_cmd( io_ports, IO_CMD_RX_UART )
    char_c = io_ports[UART_IO_PORT]
    send_cmd( io_ports, IO_CMD_NOP )
    return char_c

def tx_char( io_ports, char_c ):
#    print(chr(char_c), end='', flush=True)
    wait_freedom( io_ports )
    io_ports[UART_IO_PORT] = char_c
    send_cmd( io_ports, IO_CMD_TX_UART )
    send_cmd( io_ports, IO_CMD_NOP )

def flush( io_ports ):
    wait_freedom( io_ports )
    send_cmd( io_ports, IO_CMD_TX_FLUSH_UART )
    send_cmd( io_ports, IO_CMD_NOP )
